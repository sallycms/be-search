...
class Util implements \sly_ContainerAwareInterface {
	...
	/**
	 * Event handler for SLY_CONTROLLER_FOUND event
	 *
	 * @param mixed $controller
	 * @param array $params
	 */
	public function controllerFound($controller, array $params) {
		$dispatcher = $this->container['sly-dispatcher'];
		$page       = $params['name'];
		$assets     = null;

		if ($page === 'structure') {
			$assets = true;
			$dispatcher->addListener('PAGE_STRUCTURE_HEADER', array($this, 'articleSearch'));
		}
		elseif ($page === 'content' || $page === 'contentmeta') {
			$assets = true;
			$dispatcher->addListener('PAGE_CONTENT_HEADER', array($this, 'articleSearch'));
		}
		elseif ($page === 'mediapool') {
			$assets = false;
			$dispatcher->addListener('SLY_MEDIA_LIST_TOOLBAR', array($this, 'mediaToolbar'));
			$dispatcher->addListener('SLY_MEDIA_LIST_QUERY', array($this, 'mediaQuery'));
		}

		if ($assets !== null) {
			$dispatcher->addListener('PAGE_HEADER', array($this, 'addAssets'), array('articleSearch' => $assets));
